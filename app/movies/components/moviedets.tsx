...
export default function MovieDets(props:{id:number}){
    ...
    useEffect(()=>{const fetchdata=async()=>{
        const genreres=await fetch("https://letter-box-steel.vercel.app/movies/genres/"+props.id)
        const studiosres=await fetch("https://letter-box-steel.vercel.app/movies/studios/"+props.id)
        const countryres=await fetch("https://letter-box-steel.vercel.app/movies/countrys/"+props.id)
        const primlangres=await fetch("https://letter-box-steel.vercel.app/movies/languages/"+props.id+"/1")
        const langres=await fetch("https://letter-box-steel.vercel.app/movies/languages/"+props.id+"/10")
        const titlesres=await fetch("https://letter-box-steel.vercel.app/movies/altitles/"+props.id)
